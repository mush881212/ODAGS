...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        uid = intr.id
        R = np.transpose(qvec2rotmat(extr.qvec)) # w2c rotation.T
        T = np.array(extr.tvec) # w2c rotation
...
def readCamerasFromTransforms(path, transformsfile, white_background, extension=".png"):
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in enumerate(frames):
            ...
            cam_info = CameraInfo(uid=idx, R=R, T=T, FovY=FovY, FovX=FovX, image=image, depth = None,
                            image_path=image_path, image_name=image_name, width=image.size[0], height=image.size[1])
            cam_infos.append(cam_info)
            
    return cam_infos, trans_list
...
def readNerfSyntheticInfo(path, white_background, eval, extension=".png"):
    ...
    if not os.path.exists(ply_path):
        ...
        # add camera bounding box to the scene
        camera_trans = np.array(train_trans_list)
        x = camera_trans[:, 0]
        y = camera_trans[:, 1]
        z = camera_trans[:, 2]
        x_min, x_max = x.min() - 20, x.max() + 20
        y_min, y_max = y.min() - 20, y.max() + 20
        z_min, z_max = z.min() - 20, z.max() + 20
        print(f"{x_min} {x_max} {y_min} {y_max} {z_min} {z_max}")
        x_sample = np.random.random((num_pts, 1)) * (x_max - x_min) + x_min
        y_sample = np.random.random((num_pts, 1)) * (y_max - y_min) + y_min
        z_sample = np.random.random((num_pts, 1)) * (z_max - z_min) + z_min
        xyz = np.concatenate([x_sample, y_sample, z_sample], axis=1)
